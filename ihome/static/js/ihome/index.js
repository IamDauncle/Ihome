...
$(document).ready(function(){
    ...
    $.get('/api/1.0/houses/index',function (response) {
        if (response.errno == '0'){
            var html = template('swiper-houses-tmpl',{'houses':response.data.houses_list})
    ...
    // TODO: 获取城区信息,获取完毕之后需要设置城区按钮点击之后相关操作
     ...
     $.get('/api/1.0/areas',function (response) {
        ...
        if (response.errno == '0'){
            ...
            var html = template('area-list-tmpl',{'areas':response.data.areas_list});
            $('.area-list').html(html);
